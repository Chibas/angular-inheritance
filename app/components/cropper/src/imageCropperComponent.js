import { Component, Input, Renderer, ViewChild, Input, Output, EventEmitter } from '@angular/core';
...
var ImageCropperComponent = (function () {
    ...
    ImageCropperComponent.prototype.close_canvas = function() {
        this.showCanvas = false;
        this.showLabel = false;
    };

    ImageCropperComponent.prototype.save = function() {
        this.image.image = this.cropper.getCroppedImageHelper().src;
        this.onCrop.emit(this.cropper.getCropBounds());
        this.updateCropBounds();
        this.close_canvas();
    ...
    ImageCropperComponent.decorators = [
        { type: Component, args: [{
                    ...
                    template: `<span class="ng2-imgcrop">
                                    <label [ngClass]="{'hide_label' : !showLabel}"> <img class="img-responsive" src="{{src}}" alt="" />
                                        <input *ngIf="!settings.noFileInput" type="file" accept="image/*"                                     
                                        (change)="fileChangeListener($event)"> 
                                    </label>        
                                    <div class="canvas_wrap" [ngClass]="{'canvas_opened' : showCanvas, 'canvas_closed' : !showCanvas }">
                                        <canvas #cropcanvas
                                            (mousedown)="onMouseDown($event)" (mouseup)="onMouseUp($event)"
                                            (mousemove)="onMouseMove($event)"  (mouseleave)="onMouseUp($event)"
                                            (touchmove)="onTouchMove($event)"  (touchend)="onTouchEnd($event)"
                                            (touchstart)="onTouchStart($event)">
                                        </canvas> 
                                        <div class="canvas_actions">
                                            <a class="close_canvas" (click)="save()">OK</a>
                                            <a class="cancel_canvas" (click)="close_canvas()">Cancel</a>
                                        </div>
                                    </div>     
                                </span>`
                },] },
